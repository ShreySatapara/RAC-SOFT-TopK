...
#  Only works for finding top k values    #
###########################################
...
def sinkhorn_forward_stablized(C, mu, nu, epsilon, max_iter):
    ...
    def min_epsilon_row(Z, epsilon):
        return -epsilon * torch.logsumexp(-Z / epsilon, dim=-1, keepdim=True)

    def min_epsilon_col(Z, epsilon):
        return -epsilon * torch.logsumexp(-Z / epsilon, dim=-2, keepdim=True)

    for i in range(max_iter):
        f = min_epsilon_row(C - g, epsilon) + epsilon_log_mu
        g = min_epsilon_col(C - f, epsilon) + epsilon_log_nu

    Gamma = torch.exp((f + g - C) / epsilon)
    return Gamma
...
def sinkhorn_backward(grad_output_Gamma, Gamma, mu, nu, epsilon):
    ...
    temp = inv_Kappa.matmul(g2).transpose(-1, -2)
    G32 = F.pad(temp, pad=(0, 1), mode="constant", value=0) * Gamma  # [bs, n, k+1]
    G3 = G31 + G32  # [bs, n, k+1]
...
class TopKFunc(Function):
    @staticmethod
    def forward(ctx, C, mu, nu, epsilon, max_iter, device="cpu"):
        with torch.no_grad():
            if epsilon > 1e-2:
...
if __name__ == "__main__":
    ...
    soft_top_k = SoftTopK(5, epsilon=0.01)  #### Choose wisely!!!
    scores = torch.randint(0, 10, (1, 4)).float().cuda()
    curr = scores.size(1)
    if curr < 5:
        scores = torch.cat(
            [scores, torch.full((1, 5 - curr), float("-inf")).cuda()], dim=1
        )
    scores.requires_grad = True
    print(f"Scores: {scores}")
    A, _ = soft_top_k(scores)
    print(f"Top-k: {A}")
    print(f"Shape: {A.shape}, Scores: {scores.shape}")
    print(A.permute(0, 2, 1).matmul(scores.unsqueeze(-1)))
    print(scores.topk(5).values)
    loss = torch.norm(A - torch.ones_like(A), p=2)
    loss.backward()
    print("Gradient", scores.grad)
